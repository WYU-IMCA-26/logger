...
void Logger::init( const std::string& logFilePath = "logger.log"){
    if(m_running){
        return;
    }
...
void Logger::log(bool write = NO_WRITE, bool out = OUT, const std::string& message){
    this->Write = write;
    this->Out = out;
    if(!m_running){
        return;
